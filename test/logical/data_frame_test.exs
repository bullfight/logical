defmodule Logical.DataFrameTest do
  ...
  test "filter/2 with equal comparison" do
    ...
    predicate = %Binary{operator: "equal", field: "a", value: 2}
    df1 = LDF.filter(predicate, df)
    assert DF.to_columns(df1, atom_keys: true) == %{a: [2, 2], b: [2.4, 2.0]}

    # df2 = DF.filter_with(df, fn ldf -> Series.equal(1.0, ldf["b"]) end)
    predicate = %Binary{operator: "equal", field: "b", value: 1.0}
    df2 = LDF.filter(predicate, df)
    ...
    predicate = %Binary{operator: "equal", field: "a", value: [1, 0, 3, 0]}
    df3 = LDF.filter(predicate, df)
    ...
    predicate = %Binary{operator: "equal", field: "b", value: [0, 0, 1.0, 0]}
    df4 = LDF.filter(predicate, df)
  ...
  test "filter/2 with equal negation" do
    ...
    # df1 = DF.filter_with(df, fn ldf -> Series.equal(ldf["a"], 2) end)
    predicate = %Binary{operator: "equal", field: "a", value: 2, negate: true}
    df1 = LDF.filter(predicate, df)
  ...
  test "filter/2 with greater_than inequality" do
    ...
    predicate = %Binary{operator: "greater_than", field: "b", value: 3.0}
    df1 = LDF.filter(predicate, df)
